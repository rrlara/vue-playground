(function(GeoJSON) {
  ...
  function buildGeom(item, params) {
    ...
    for(var gtype in params.geom) {
      ...
      if(typeof val === 'string' && item.attributes.hasOwnProperty(val)) {
        geom.type = gtype;
        geom.coordinates = item[val];
      }

      // Geometry parameter specified as: {Point: ['lat', 'lng']}
      else if(Array.isArray(val) && item.attributes.hasOwnProperty(val[0]) && item.attributes.hasOwnProperty(val[1])){
        geom.type = gtype;
        if (typeof val[1] === 'string' && typeof val[0] === 'string' ) {
          geom.coordinates = [ parseFloat(item.attributes[val[1]]), parseFloat(item.attributes[val[0]]) ];
        }else{
          geom.coordinates = [ item.attributes[val[1]], item.attributes[val[0]] ];
        }
